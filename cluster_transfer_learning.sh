...
cd $ROOT_DIR
for i in {1..5}; do
    ITERATION_FOLDER="$NODE_RESULTADOS_DIR/iteration_$i"
    check_command_result mkdir -p $ITERATION_FOLDER
    check_command_result python main.py train --dataset_dir $NODE_DATASET_DIR/$DATASET_NAME1 --logs_dir $ITERATION_FOLDER \
        --batch_size $BATCHSIZE --tile_size $TILESIZE --encoder resnet18 --number_of_epochs $EPOCHS \
        --boundary_thickness 3 --augmentation 1 --test_size 0 --model_type $MODEL_TYPE --freeze_encoder 1

    MODEL_WEIGHTS=$(find ${ITERATION_FOLDER} -name best_model.pth)
    check_command_result python main.py train --dataset_dir $NODE_DATASET_DIR/$DATASET_NAME2 --logs_dir $ITERATION_FOLDER \
        --batch_size $BATCHSIZE --tile_size $TILESIZE --encoder resnet18 --number_of_epochs $EPOCHS \
    ...
    check_command_result cp -r $ITERATION_FOLDER/* $HOME_RESULTADOS_DIR
done
